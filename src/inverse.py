...
def inverse(meas,dict,mix):
    ...
    if dict["start_points"]>1:
        ...
        for i in range(dict["start_points"]):
            ...
            if dict["method"] == "Root":
                result = scipy.optimize.root(inverse_minimize,preshock_state_var,args=(meas,dict,mix,dict["method"]),tol=dict["residual"])
            elif dict["method"] == "Hybrid":
                result = scipy.optimize.minimize(inverse_minimize,preshock_state_var,args=(meas,dict,mix,dict["method"]),method="L-BFGS-B",tol=1.0e-03,options=options)
                preshock_state_var = result.x
                result = scipy.optimize.root(inverse_minimize,preshock_state_var,args=(meas,dict,mix,dict["method"]),tol=dict["residual"])
            else:
                result = scipy.optimize.minimize(inverse_minimize,preshock_state_var,args=(meas,dict,mix,dict["method"]),method=dict["method"],tol=dict["residual"],bounds=bnds,options=options)
            evals[i] = result.fun
        ...
        if dict["method"] == "Root":
            ...
            result = scipy.optimize.minimize(inverse_minimize,preshock_state_var,args=(meas,dict,mix,dict["method"]),method=dict["method"],tol=dict["residual"],options=options)
        print(result.message)
